...
def transport():
    ...
    # ------------------------------------------------------------------------------------------
    t = ttk.Label(root, text="Welcome to Finding-Business.io", font=50, style='Custom.TLabel')
    t.pack()
    ...
    t.pack()
    # ----------------------------------------------------------------------------------------
    input_frame = ttk.Frame(root, padding=(20, 10, 20, 0), style='Custom.TLabel')
    ...
    location_entry = tkinter.Entry(input_frame, width=50, textvariable=location_String)
    ...
    def des():
        root.destroy()
    quit_button = ttk.Button(button_frame, text="Close the App", command=lambda:[des(),User_Choice()])
...
def review():
    ...
    def Option2nd():
        ...
        if avg_reviews > 50:
            ...
            '''
            desired_output += f"Based on our algorithm,the footfall of area seems good'\n" \
                              f"This location is suitable for opening a {business_type} store.'\n" \
    ...
    type1_entry = tkinter.Entry(input_frame, width=50, textvariable=type1_String)
    type1_entry.grid(row=1, column=1, pady=(0, 10))
    # ----------------------------------------------------------------------------------------
    '''
    option1st_button = ttk.Button(root, text="1. Get list of similar business in area", command=Option1st)
    ...
    style.configure('Custom.TFrame', background='#ffbf00')
    button_frame = ttk.Frame(root, style='Custom.TFrame')
    option1st_button = ttk.Button(button_frame, text="2. Find if the location is suitable (on the basis of footfall)",
                                  command=Option2nd)
    option1st_button.pack(side="left", fill="x", expand=True, padx=(0, 10))
    # ----------------------------------------------------------------------------------------
    def des():
...
def near_Bus():
    ...
    def des():
        root.destroy()
    quit_button = ttk.Button(root, text="Close the App", command=lambda:[des(),User_Choice()])
    quit_button.pack()
    root.mainloop()
...
def User_Choice():
    ...
    L4.image=photo2
    L4.pack()
    Bu1=Button(T,text="1. Search Similar Business",command=lambda:[des(),near_Bus()],bg="#7FFFD4",font=("Algerian",15),bd=5,activebackground="#00BFFF")
    Bu1.place(x=20,y=50)
    Bu2=Button(T,text="2. CHECK POPULARITY OF LOCATION",command=lambda:[des(),review()],bg="#7FFFD4",font=("Algerian",15),bd=5,activebackground="#00BFFF")
